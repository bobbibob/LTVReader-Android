# Полезные ссылки

## Оригинал
- [LocalText2Voice (upstream)](https://github.com/estebanstifli/LocalText2Voice) — оригинальный репозиторий
- [bobbibob/Text2Voice (fork)](https://github.com/bobbibob/Text2Voice) — форк, с которого сделан порт
- [AndromedaNova.com](https://andromedanova.com) — сайт автора

## TTS-движки
- [Kokoro-ONNX](https://github.com/hexgrad/kokoro-onnx) — локальный движок
- [Kokoro на HuggingFace](https://huggingface.co/onnxcommunity/Kokoro) — модели
- [Piper TTS](https://github.com/rhasspy/piper) — быстрый CPU-движок
- [Chatterbox TTS](https://github.com/resemble-ai/chatterbox) — voice cloning
- [Qwen3 TTS](https://github.com/QwenLM/Qwen3) — мультиязычный
- [OmniVoice](https://github.com/...) — voice design

## Облачные API
- [OpenAI TTS](https://platform.openai.com/docs/guides/text-to-speech)
- [ElevenLabs](https://elevenlabs.io/docs/api-reference/text-to-speech)
- [Google Gemini TTS](https://ai.google.dev/gemini-api/docs/speech-generation)
- [Azure Speech](https://learn.microsoft.com/azure/ai-services/speech-service/rest-text-to-speech)

## Android-стек
- [Jetpack Compose](https://developer.android.com/jetpack/compose)
- [Material 3](https://m3.material.io/)
- [Room](https://developer.android.com/training/data-storage/room)
- [DataStore](https://developer.android.com/topic/libraries/architecture/datastore)
- [WorkManager](https://developer.android.com/topic/libraries/architecture/workmanager)

## Библиотеки
- [ONNX Runtime Android](https://onnxruntime.ai/docs/install/android.html)
- [ffmpeg-kit](https://github.com/Arthenica/ffmpeg-kit) — ffmpeg для Android
- [OkHttp](https://square.github.io/okhttp/)
- [kotlinx.serialization](https://github.com/Kotlin/kotlinx.serialization)
- [kotlinx.coroutines](https://github.com/Kotlin/kotlinx.coroutines)

## Алгоритмы и стандарты
- [EBU R128](https://tech.ebu.ch/publications/r128) — нормализация громкости
- [WebVTT](https://www.w3.org/TR/webvtt1/) — формат субтитров
- [SubRip (SRT)](https://docs.fileformat.com/video/srt/) — формат субтитров
- [SSML](https://www.w3.org/TR/speech-synthesis/) — разметка речи
- [BCP-47](https://tools.ietf.org/html/bcp47) — языковые теги

## Документация проекта
- [README](../README.md) — обзор
- [PORTING](PORTING.md) — что и почему портировано
- [ROADMAP](ROADMAP.md) — что сделано, что в работе
- [LTV_MARKUP](LTV_MARKUP.md) — поведение разметки
- [ARCHITECTURE](ARCHITECTURE.md) — диаграммы
- [INTERNALS](INTERNALS.md) — как устроено внутри
- [QUICKSTART](QUICKSTART.md) — быстрый старт
- [TESTING](TESTING.md) — тестирование
- [DEPLOYMENT](DEPLOYMENT.md) — публикация
- [TROUBLESHOOTING](TROUBLESHOOTING.md) — решение проблем
- [FAQ](FAQ.md) — частые вопросы
- [EXTENDING](EXTENDING.md) — как добавлять фичи
- [SECURITY](SECURITY.md) — безопасность
- [PERFORMANCE](PERFORMANCE.md) — производительность
- [COMPARISON](COMPARISON.md) — сравнение с конкурентами
- [MODELS](MODELS.md) — какие движки выбрать
- [EN_ENGINE_HOST](EN_ENGINE_HOST.md) — гайд по server-host
- [MARKETING](MARKETING.md) — продвижение
- [VERSIONING](VERSIONING.md) — версионирование

